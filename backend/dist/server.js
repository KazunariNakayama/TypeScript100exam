...
const runExecutor_1 = require("./runExecutor");
...
// 問題ファイルのディレクトリパスを設定
const problemsDir = path_1.default.join(__dirname, '../problems');
// Get list of problems
app.get('/api/problems', async (req, res) => {
    try {
        console.log('Reading problems from:', problemsDir);
        const files = await promises_1.default.readdir(problemsDir);
        console.log('Found files:', files);
        const problems = await Promise.all(files
            .filter(file => file.endsWith('.json'))
            .map(async (file) => {
            const filePath = path_1.default.join(problemsDir, file);
...
app.get('/api/problems/:id', async (req, res) => {
    try {
        ...
        const filePath = path_1.default.join(problemsDir, `${problemId}.json`);
...
app.post('/api/run', async (req, res) => {
    try {
        ...
        const result = await (0, runExecutor_1.runCode)(code, testCode);
        res.json(result);
    }
    catch (error) {
